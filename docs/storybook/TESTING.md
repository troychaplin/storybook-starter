...
This guide walks through verifying that the Storybook starter project and its `component2block` integration are working correctly.
...
# From the component2block folder
cd component2block
npm install
npm run build
cd ..
...
## 1. Run the component2block Unit Tests
...
```
component2block: generated files:
...
- [ ] File starts with `/* Auto-generated by component2block — do not edit manually */`
- [ ] All variables use the `--prefix-` prefix
- [ ] Variables are grouped by category (Colors, Spacing, Font Families, etc.)
- [ ] All values match what's in `c2b.config.json`
...
- [ ] Changing a value in `c2b.config.json` and restarting `npm run dev` updates the component styles

## 5. Test Dry Run Mode

```bash
node component2block/dist/cli.js generate --dry-run
```
...
node component2block/dist/cli.js generate --config /tmp/bad-config.json
```
...
```bash
node component2block/dist/cli.js generate --config /tmp/nonexistent.json
```
