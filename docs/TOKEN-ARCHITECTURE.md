...
A single configuration file (`config/tokens.json`) serves as the source of truth for all design values. A build script generates platform-specific outputs from this config, ensuring that every consumer — Storybook, a React app, or a WordPress block theme — shares the same design values without manual synchronization.

```
config/tokens.json                  (single source of truth)
    │
    │   scripts/generate-tokens.js  (build script)
    │
    ├──► src/styles/tokens.css      (CSS vars for Storybook dev)
    ├──► dist/css/tokens.css        (CSS vars for React/Next.js consumers)
    ├──► dist/theme.json            (WordPress theme.json base layer)
    └──► dist/integrate.php         (WordPress filter hook)
```
...
```css
/* Auto-generated from config/tokens.json — do not edit manually */
...
#### Step 1: Include the integration hook
...
```json
{
    "exports": {
        ...
        "./integrate.php": "./dist/integrate.php"
    }
}
```
...
The generation script reads `config/tokens.json` and produces:

1. `src/styles/tokens.css` — for Storybook development
2. `dist/css/tokens.css` — for React/Next.js consumers
3. `dist/css/tokens.wp.css` — for WordPress consumers
4. `dist/theme.json` — WordPress theme.json base layer
5. `dist/integrate.php` — WordPress filter hook (static file, copied)
...
The `generate` step runs first, ensuring `tokens.css` exists before Storybook or Vite needs it.

## Adding New Tokens

1. Add the token to `config/tokens.json`
...
Update the `prefix` value in `config/tokens.json`:
