...
The `story-to-block` Storybook preset auto-injects all generated and authored style files into Storybook. No manual imports needed in `preview.ts`.
...
The preset reads `stb.config.json`, derives file paths from `tokensPath`, and injects any that exist:
